...
seq1 = fileIn.readline().strip()
seq2 = fileIn.readline().strip()
...
def localAlignment(seq1, seq2):
    ...
    for i, row in enumerate(scoreMatrix[1:], start=1):
        for j, column in enumerate(row[1:], start=1):
            match = scoreMatrix[i-1][j-1] + matrix[seq1[i-1]][seq2[j-1]]
            insertion = scoreMatrix[i-1][j] - penalty
            deletion = scoreMatrix[i][j-1] - penalty
            scoreMatrix[i][j] = max(0, match, insertion, deletion)
            backTrack[i][j] = [insertion, deletion, match, 0].index(scoreMatrix[i][j])
            
            if scoreMatrix[i][j] > maximumScore:
                maximumScore = scoreMatrix[i][j]
                maxA = i 
                maxB = j

    def insertIndel(seq, i):
        return seq[:i] + '-' + seq[i:]
    ...
    while a * b != 0 and backTrack[a][b] != 3 :
        if backTrack[a][b] == 0:
            a -= 1
            s2 = insertIndel(s2, b)
    ...
    result = (maximumScore, s1[a:], s2[b:])
    return result

alignment = localAlignment(seq1, seq2)
